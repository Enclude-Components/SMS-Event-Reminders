...
The SMS body is built in the [SUBFLOW_Send_Event_SMS_Notification](./force-app/main/default/flows/SUBFLOW_Send_Event_SMS_Notification.flow-meta.xml) flow using the `DateTimeFormatterInvocable` Apex action, which returns the event's start and end times as separate date (`DD/MM/YYYY`) and time (`HH:mm`) strings in Irish time (Europe/Dublin). These can be composed freely in a Flow Text Template to produce whatever message format is required.
